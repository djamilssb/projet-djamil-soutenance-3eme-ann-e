...
describe("SaveQuizzRepository", () => {
    ...
    it("should retrieve all quiz saves", async () => {
        ...
        expect(executeQuery).toHaveBeenCalledWith("SELECT * FROM Save_quizz", []);
    ...
    it("should retrieve a quiz save by ID", async () => {
        ...
        expect(executeQuery).toHaveBeenCalledWith("SELECT * FROM Save_quizz WHERE id = ?", [1]);
    ...
    it("should return null if quiz save is not found", async () => {
        ...
        expect(executeQuery).toHaveBeenCalledWith("SELECT * FROM Save_quizz WHERE id = ?", [999]);
    ...
    it("should create a quiz save", async () => {
        ...
        expect(executeQuery).toHaveBeenCalledWith("INSERT INTO Save_quizz SET ?", [saveQuizz]);
    ...
    it("should update a quiz save", async () => {
        ...
        expect(executeQuery).toHaveBeenCalledWith(
            "UPDATE Save_quizz SET ? WHERE id = ? AND id_user = ? AND id_quizz = ?",
            [{ score: 100 }, 1, 1, 1]
        );
        expect(result).toBe(true);
    });
    ...
    it("should delete a quiz save", async () => {
        ...
        expect(executeQuery).toHaveBeenCalledWith("DELETE FROM Save_quizz WHERE id = ?", [1]);
        expect(result).toBe(true);
    });
});